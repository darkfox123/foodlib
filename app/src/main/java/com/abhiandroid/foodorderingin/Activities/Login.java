...
public class Login extends AppCompatActivity {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.signIn:
                if (Config.validateEmail(email,Login.this) && validatePassword(password)) {
                    login();
                }
                break;
        }
    ...
    private void login() {
        ...
        String req = "{\"res_id\":\"res007\",\"email\":\""+email.getText().toString().trim()+"\",\"password\":\""+ password.getText().toString().trim()+"\",\"logintype\":\""+ "email" +"\"}";
        Volley_Request postRequest = new Volley_Request();
        postRequest.createRequest(Login.this, getResources().getString(R.string.mJSONURL_login), "POST", "login", req);

        /*
        Api.getClient().login("res007", editTexts.get(0).getText().toString().trim(),
    ...
    public static void loginResponse(String response){
        try {
            JSONObject jObj = new JSONObject(response);
            ps.dismiss();
            Log.d("signUpResponse", jObj.getString("user_id") + "");
            Toast.makeText(mContext, jObj.getString("message"), Toast.LENGTH_SHORT).show();
            if (jObj.getString("success").equalsIgnoreCase("true")) {
                Common.saveUserData(mContext, "email", password.getText().toString());
                Common.saveUserData(mContext, "userId", jObj.getString("user_id") + "");
                Config.moveTo(mContext, MainActivity.class);
                mActivity.finishAffinity();
            } else if (jObj.getString("success").equalsIgnoreCase("notactive")) {
